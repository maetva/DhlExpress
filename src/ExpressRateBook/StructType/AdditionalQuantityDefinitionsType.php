...
class AdditionalQuantityDefinitionsType extends AbstractStructBase
{
     ...
     * - maxOccurs: 100
     * - minOccurs: 0
     * @var \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType[]
     */
     ...
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType[] $additionalQuantityDefinition
     */
    ...
    public static function validateAdditionalQuantityDefinitionForArrayConstraintsFromSetAdditionalQuantityDefinition(?array $values = []): string
    {
        ...
        foreach ($values as $additionalQuantityDefinitionsTypeAdditionalQuantityDefinitionItem) {
            ...
            if (!$additionalQuantityDefinitionsTypeAdditionalQuantityDefinitionItem instanceof \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AdditionalQuantityDefinition property can only contain items of type \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType[] $additionalQuantityDefinition
     * @return \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionsType
     */
     ...
     * @throws InvalidArgumentException
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType $item
     * @return \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionsType
     */
    public function addToAdditionalQuantityDefinition(\Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType) {
            throw new InvalidArgumentException(sprintf('The AdditionalQuantityDefinition property can only contain items of type \Maetva\DhlExpress\ExpressRateBook\StructType\AdditionalQuantityDefinitionType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(100)
