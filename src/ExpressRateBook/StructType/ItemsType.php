...
class ItemsType extends AbstractStructBase
{
     ...
     * - maxOccurs: 1000
     * @var \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType[]
     */
    protected ?array $Item = null;
    /**
     * Constructor method for ItemsType
     * @uses ItemsType::setItem()
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType[] $item
     */
    ...
    public static function validateItemForArrayConstraintsFromSetItem(?array $values = []): string
    {
        ...
        foreach ($values as $itemsTypeItemItem) {
            ...
            if (!$itemsTypeItemItem instanceof \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Item property can only contain items of type \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType[] $item
     * @return \Maetva\DhlExpress\ExpressRateBook\StructType\ItemsType
     */
     ...
     * @return \Maetva\DhlExpress\ExpressRateBook\StructType\ItemsType
     */
    public function addToItem(\Maetva\DhlExpress\ExpressRateBook\StructType\ItemType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType) {
            throw new InvalidArgumentException(sprintf('The Item property can only contain items of type \Maetva\DhlExpress\ExpressRateBook\StructType\ItemType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(1000)
