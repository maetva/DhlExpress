...
class DocTypeRef_Documents extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document[]
     */
    protected ?array $Document = null;
    /**
     * Constructor method for docTypeRef_Documents
     * @uses DocTypeRef_Documents::setDocument()
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document[] $document
     */
    ...
    public static function validateDocumentForArrayConstraintsFromSetDocument(?array $values = []): string
    {
        ...
        foreach ($values as $docTypeRef_DocumentsDocumentItem) {
            ...
            if (!$docTypeRef_DocumentsDocumentItem instanceof \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Document property can only contain items of type \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document[] $document
     * @return \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Documents
     */
     ...
     * @throws InvalidArgumentException
     * @param \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document $item
     * @return \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Documents
     */
    public function addToDocument(\Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document) {
            throw new InvalidArgumentException(sprintf('The Document property can only contain items of type \Maetva\DhlExpress\ExpressRateBook\StructType\DocTypeRef_Document, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
