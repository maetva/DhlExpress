...
class ShipmentMonetaryAmountType extends AbstractStructBase
{
     ...
     * - maxOccurs: 20
     * @var \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType[]
     */
    protected ?array $Charges = null;
    /**
     * Constructor method for ShipmentMonetaryAmountType
     * @uses ShipmentMonetaryAmountType::setCharges()
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType[] $charges
     */
    ...
    /**
     * Get Charges value
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType[]
     */
    ...
    public static function validateChargesForArrayConstraintsFromSetCharges(?array $values = []): string
    {
        ...
        foreach ($values as $shipmentMonetaryAmountTypeChargesItem) {
            ...
            if (!$shipmentMonetaryAmountTypeChargesItem instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Charges property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType[] $charges
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ShipmentMonetaryAmountType
     */
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType $item
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ShipmentMonetaryAmountType
     */
    public function addToCharges(\Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType) {
            throw new InvalidArgumentException(sprintf('The Charges property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargesType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(20)
