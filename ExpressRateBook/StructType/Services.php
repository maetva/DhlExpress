...
class Services extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * @var \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service[]
     */
    protected ?array $Service = null;
    /**
     * Constructor method for Services
     * @uses Services::setService()
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service[] $service
     */
    ...
    public static function validateServiceForArrayConstraintsFromSetService(?array $values = []): string
    {
        ...
        foreach ($values as $servicesServiceItem) {
            ...
            if (!$servicesServiceItem instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Service property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service[] $service
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Services
     */
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service $item
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Services
     */
    public function addToService(\Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service) {
            throw new InvalidArgumentException(sprintf('The Service property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
