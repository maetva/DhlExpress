...
class GoodsCharacteristicsType extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * @var \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType[]
     */
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType[] $goodsCharacteristic
     */
    ...
    public static function validateGoodsCharacteristicForArrayConstraintsFromSetGoodsCharacteristic(?array $values = []): string
    {
        ...
        foreach ($values as $goodsCharacteristicsTypeGoodsCharacteristicItem) {
            ...
            if (!$goodsCharacteristicsTypeGoodsCharacteristicItem instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The GoodsCharacteristic property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType $item
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicsType
     */
    public function addToGoodsCharacteristic(\Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType) {
            throw new InvalidArgumentException(sprintf('The GoodsCharacteristic property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
