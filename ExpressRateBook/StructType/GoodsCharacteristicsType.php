...
class GoodsCharacteristicsType extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * @var \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType[]
     */
     ...
     * @param \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType[] $goodsCharacteristic
     */
    ...
    public static function validateGoodsCharacteristicForArrayConstraintsFromSetGoodsCharacteristic(?array $values = []): string
    {
        ...
        foreach ($values as $goodsCharacteristicsTypeGoodsCharacteristicItem) {
            ...
            if (!$goodsCharacteristicsTypeGoodsCharacteristicItem instanceof \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The GoodsCharacteristic property can only contain items of type \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType $item
     * @return \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicsType
     */
    public function addToGoodsCharacteristic(\Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType) {
            throw new InvalidArgumentException(sprintf('The GoodsCharacteristic property can only contain items of type \Maetva\DhlExpress\Endpoint\ExpressRateBook\StructType\GoodsCharacteristicType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
