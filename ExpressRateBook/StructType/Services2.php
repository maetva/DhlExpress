...
class Services2 extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * @var \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2[]
     */
    protected ?array $Service = null;
    /**
     * Constructor method for Services2
     * @uses Services2::setService()
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2[] $service
     */
    ...
    public static function validateServiceForArrayConstraintsFromSetService(?array $values = []): string
    {
        ...
        foreach ($values as $services2ServiceItem) {
            ...
            if (!$services2ServiceItem instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Service property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2[] $service
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Services2
     */
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2 $item
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Services2
     */
    public function addToService(\Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2 $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2) {
            throw new InvalidArgumentException(sprintf('The Service property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Service2, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
