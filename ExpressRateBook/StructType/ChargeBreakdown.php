...
class ChargeBreakdown extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * - minOccurs: 0
     * @var \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown[]
     */
    protected ?array $Breakdown = null;
    /**
     * Constructor method for ChargeBreakdown
     * @uses ChargeBreakdown::setBreakdown()
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown[] $breakdown
     */
    ...
    public static function validateBreakdownForArrayConstraintsFromSetBreakdown(?array $values = []): string
    {
        ...
        foreach ($values as $chargeBreakdownBreakdownItem) {
            ...
            if (!$chargeBreakdownBreakdownItem instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Breakdown property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown[] $breakdown
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargeBreakdown
     */
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown $item
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ChargeBreakdown
     */
    public function addToBreakdown(\Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown) {
            throw new InvalidArgumentException(sprintf('The Breakdown property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\Breakdown, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
