...
class DocTypeRef_ExchangeRateType extends AbstractStructBase
{
     ...
     * - maxOccurs: 3
     * @var \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate[]
     */
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate[] $exchangeRate
     */
    ...
    public static function validateExchangeRateForArrayConstraintsFromSetExchangeRate(?array $values = []): string
    {
        ...
        foreach ($values as $docTypeRef_ExchangeRateTypeExchangeRateItem) {
            ...
            if (!$docTypeRef_ExchangeRateTypeExchangeRateItem instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExchangeRate property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate[] $exchangeRate
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\DocTypeRef_ExchangeRateType
     */
     ...
     * @throws InvalidArgumentException
     * @param \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate $item
     * @return \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\DocTypeRef_ExchangeRateType
     */
    public function addToExchangeRate(\Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate) {
            throw new InvalidArgumentException(sprintf('The ExchangeRate property can only contain items of type \Drupal\commerce_dhl_express\Endpoint\ExpressRateBook\StructType\ExchangeRate, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(3)
